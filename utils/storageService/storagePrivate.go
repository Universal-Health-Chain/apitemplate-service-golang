...
func (p *PrivateStorage) Put(vaultID string, documents ...storageUtils.EncryptedDocument) error {
	mongoDBStore, ok := p.documentsStore.(*ariesStorageMongodb.Store)
	if ok {
		return storageUtils.StoreDocumentsForMongoDB(vaultID, documents, mongoDBStore)
	}
	...
	for i := 0; i < len(documents); i++ {
		...
		operations[i].Key = storageUtils.GenerateAriesDocumentEntryKey(vaultID, documents[i].ID)
		operations[i].Value = documentBytes
		operations[i].Tags = storageUtils.CreateTags(vaultID, documents[i])
	}
...
func (p *PrivateStorage) Get(vaultID, documentID string) ([]byte, error) {
	...
	return p.documentsStore.Get(storageUtils.GenerateAriesDocumentEntryKey(vaultID, documentID))
}
...
func (p *PrivateStorage) Delete(vaultID, documentID string) error {
	mongoDBStore, ok := p.documentsStore.(*ariesStorageMongodb.Store)
	if ok {
		filter := bson.M{storageUtils.DocumentIDFieldName: documentID, storageUtils.VaultIDTagName: vaultID}
	...
	return p.documentsStore.Delete(storageUtils.GenerateAriesDocumentEntryKey(vaultID, documentID))
}
...
func (p *PrivateStorage) Query(vaultID string, query storageUtils.Query) ([]storageUtils.EncryptedDocument, error) {
	mongoDBStore, ok := p.documentsStore.(*ariesStorageMongodb.Store)
	if ok {
		return p.queryFromMongoDB(mongoDBStore, vaultID, query)
	}

	ariesQuery, err := storageUtils.ConvertEDVQueryToAriesQuery(query)
	if err != nil {
		return nil, err
	}
...
func (p *PrivateStorage) queryForEncryptedDocumentsFromAries(vaultID, ariesQuery string) ([]storageUtils.EncryptedDocument, error) {
	...
	var encryptedDocuments []storageUtils.EncryptedDocument

	for moreEntries {
		isForCorrectVault, err := storageUtils.VaultIDTagMatches(vaultID, iterator)
		if err != nil {
			return nil, err
		}
...
func (p *PrivateStorage) queryFromMongoDB(store *ariesStorageMongodb.Store, vaultID string, query storageUtils.Query) ([]storageUtils.EncryptedDocument, error) {
	mongoDBQuery := storageUtils.ConvertEDVQueryToMongoDBQuery(vaultID, query)

	return p.queryForEncryptedDocumentsFromMongoDB(store, mongoDBQuery)
}

func (p *PrivateStorage) queryForEncryptedDocumentsFromMongoDB(store *ariesStorageMongodb.Store, filter interface{}) ([]storageUtils.EncryptedDocument, error) {
...
func (p *PrivateStorage) getFromMongoDB(store *ariesStorageMongodb.Store, vaultID, documentID string) ([]byte, error) {
	filter := bson.D{
		{Key: storageUtils.DocumentIDFieldName, Value: documentID},
		{Key: storageUtils.VaultIDTagName, Value: vaultID},
	}
